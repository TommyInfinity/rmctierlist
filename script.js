// Color palette
const colors = [
    '#FF7F7F', '#FFB347', '#FFD700', '#90EE90', '#87CEEB',
    '#DDA0DD', '#FF69B4', '#F08080', '#FF6347', '#FFA500',
    '#FFD700', '#7FFF00', '#40E0D0', '#BA55D3', '#FF1493',
    '#DC143C', '#FF8C00', '#FFB90F', '#00FA9A', '#1E90FF'
];
...
// kis “csillag/particle” objektumok
const bgParticles = [];
...
function drawBackground(now) {
    ...
    // extra irizáló “hurok” az egér körül
...
const itemDefinitions = [
    ...
    {
        id: 'trashhorror',
        name: 'Trash Horror Collection 5',
        steamAppId: null,
        customImageUrl: 'https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/3569980/ccbcacc6cac07f1153ac8d72d09aecfc4042ffaf/header.jpg?t=1742368684'
    }
];
    ...
    if (items.length > 0) {
        itemsHTML =
            `<div class="confirm-tier-info">
                <div class="confirm-tier-label" style="background-color: ${tierColor};">
                    ${tierLabel}
                </div>
            </div>
...
/* ===================== Elérhető elemek ablak – drag/resize ===================== */
        ...
        function initResize(e) {
            ...
            function resize(e) {
                ...
                if (currentResizer.classList.contains('br')) {
                    cardsContainer.style.width = rect.width + (e.clientX - prevX) + 'px';
                    cardsContainer.style.height = rect.height + (e.clientY - prevY) + 'px';
                } else if (currentResizer.classList.contains('bl')) {
                    cardsContainer.style.width = rect.width - (e.clientX - prevX) + 'px';
                    cardsContainer.style.height = rect.height + (e.clientY - prevY) + 'px';
                    cardsContainer.style.left = rect.left + (e.clientX - prevX) + 'px';
                } else if (currentResizer.classList.contains('tr')) {
                    cardsContainer.style.width = rect.width + (e.clientX - prevX) + 'px';
                    cardsContainer.style.height = rect.height - (e.clientY - prevY) + 'px';
                    cardsContainer.style.top = rect.top + (e.clientY - prevY) + 'px';
...
function addTier(label = null, color = null) {
    ...
    tier.innerHTML = `
        <div class="tier-label" style="background-color: ${tierColor};">
            ${tierLabel}
        </div>
...
function saveTierSettings() {
    ...
    if (selectedColor) {
        currentEditingTier.querySelector('.tier-label').style.backgroundColor =
...
function getTierInfo(tier) {
    ...
    const items = Array.from(tierItems.querySelectorAll('.card-item')).map(item => item.innerText.trim());
...
function clearTier(button) {
    ...
    showConfirmModal(
        'Tier ürítése',
        `Biztosan törölni szeretnéd az összes elemet a(z) ${tierInfo.tierLabel} tier-ből?`,
        tierInfo.items,
        tierInfo.tierLabel,
        tierInfo.tierColor,
        () => {
...
function deleteTier(button) {
    ...
    showConfirmModal(
        'Tier törlése',
        `Biztosan törölni szeretnéd a(z) ${tierInfo.tierLabel} tier-t? Az elemek visszakerülnek az elérhető elemek közé.`,
        tierInfo.items,
        tierInfo.tierLabel,
        tierInfo.tierColor,
        () => {
    ...
    cardsContainer.innerHTML = `
        <div class="cards-window-header">
            ...
            <div class="cards-window-controls">
                <button onclick="toggleMinimize()" title="Minimalizálás">
